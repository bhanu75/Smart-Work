  ...
  return (
    <div className="bg-slate-800/50 rounded-xl p-4 mb-3">
      ...
      <div className={`mb-2 ${getResultColor()} font-medium`}>
        {trade.result === 'RULE BROKEN' && <AlertTriangle className="w-4 h-4 inline mr-2" />}
        Result: {trade.result} {trade.points} / RR: {trade.rr} {trade.lots}
      </div>
...
const AddTradeModal = ({ isOpen, onClose, onSave, editTrade }) => {
  const [formData, setFormData] = useState({
    ...
    result: 'WIN',
    conditions: [
      { name: 'Condition 1', confirmed: true },
      { name: 'Condition 2', confirmed: true },
      { name: 'Condition 3', confirmed: true }
    ]
  });
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="BankNifty, Nifty, etc."
                required
              />
            </div>
            ...
            <div>
              ...
              >
                ...
                <option value="RULE BROKEN">RULE BROKEN</option>
              </select>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                <label className="block text-sm text-slate-300 mb-1">R:R</label>
                <input
                  type="text"
                  value={formData.rr}
              ...
              <div>
                <label className="block text-sm text-slate-300 mb-1">Lots</label>
                <input
                  type="text"
                  value={formData.lots}
...
const CalendarView = ({ trades, selectedDate, onDateSelect }) => {
  const getDaysInMonth = () => {
    const days = [];
    for (let i = 1; i <= 30; i++) {
      const dayTrades = trades.filter(t => t.date.includes(`${i}`));
      days.push({
        date: i,
  ...
  return (
    <div className="px-4 pt-6 pb-24">
      <h2 className="text-2xl font-bold mb-4">April 2024</h2>
...
const StatsView = ({ trades }) => {
  ...
  const broken = trades.filter(t => t.result === 'RULE BROKEN');
  
  const totalPoints = trades.reduce((sum, t) => {
    const points = parseInt(t.points.replace(/[^0-9-]/g, '')) || 0;
    return sum + points;
  }, 0);
    ...
    <div className="px-4 pt-6 pb-24">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        ...
        <div className="bg-slate-800 rounded-xl p-4">
          ...
          <div className={`text-3xl font-bold ${totalPoints >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
            {totalPoints >= 0 ? '+' : ''}{totalPoints}
          </div>
        </div>
        
        <div className="bg-slate-800 rounded-xl p-4">
          <div className="text-slate-400 text-sm mb-1">Rules Broken</div>
          <div className="text-3xl font-bold text-yellow-400">{broken.length}</div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="bg-yellow-500/20 rounded-xl p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <AlertTriangle className="w-8 h-8 text-yellow-400" />
            <div>
              <div className="text-slate-300 text-sm">Rule Broken</div>
              <div className="text-2xl font-bold text-yellow-400">{broken.length}</div>
            </div>
          </div>
        </div>
      </div>
...
export default function App() {
          ...
          setTrades([
            {
              ...
              conditions: [
                { name: 'Condition 1', confirmed: true },
                { name: 'Condition 2', confirmed: true },
                { name: 'Condition 3', confirmed: false }
              ],
              result: 'WIN',
              points: '+280pts',
              rr: '1:2',
              lots: '-1',
              reason: 'Breakout on Resistance, Strong Volume.'
            },
            {
              id: 2,
              date: 'April 24, 2024',
              ...
              rr: '1:1.5',
              lots: '',
              reason: 'Entered before all conditions were confirmed.'
            },
            {
              id: 3,
              date: 'April 24, 2024',
              ...
              rr: '1:2',
              lots: '-2',
              reason: 'Clean breakdown with volume confirmation.'
            }
          ]);
        }
      } catch (error) {
  ...
  const weekDays = [
    ...
    { day: 'W', date: 22 },
    { day: 'T', date: 23 },
    { day: 'F', date: 24, hasMultiple: true },
    { day: 'S', date: 25 },
    { day: 'S', date: 26 }
  ];
  ...
  const stats = {
    ...
    broken: trades.filter(t => t.result === 'RULE BROKEN').length
  };

  const handleAddTrade = (tradeData) => {
    if (editingTrade) {
      setTrades(trades.map(t => t.id === editingTrade.id ? {...tradeData, id: editingTrade.id, date: 'April 24, 2024'} : t));
  ...
  const renderContent = () => {
    ...
    if (activeTab === 'calendar') {
      return <CalendarView trades={trades} selectedDate={selectedDate} onDateSelect={setSelectedDate} />;
    }
        ...
        <div className="px-4 pt-6 pb-4">
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-lg font-medium">Today's Trades:</h2>
              <span className="text-slate-400 text-sm">● {trades.length} trades</span>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              ...
              <div className="bg-yellow-500/20 text-yellow-400 rounded-lg p-3">
                ...
                <div className="text-xs opacity-75">Broken</div>
                <div className="text-xl font-bold">{stats.broken}</div>
              </div>
            </div>
